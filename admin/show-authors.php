...
class tp_show_authors_page {
    ...
    private static function get_table ($action, $checkbox, $page, $search, $curr_page, $entry_limit, $number_messages) {
        ...
        foreach ( $row as $row ) {
            ...
            echo '<td><a class="tp_show_pub_info" onclick="javascript:teachpress_del_node(\'#pub_info_'.$row['author_id'].'\');" author_id="' . $row['author_id'] . '" title="' . __('Show publications','teachpress') . '" style_class="' . $tr_class . '" style="cursor:pointer;"><b>' . tp_bibtex::parse_author_simple($row['name']) . '</b></a>';
